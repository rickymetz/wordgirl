...
Sixfold works in two stages. A frozen schedule gives the day's letter family: the seed `anagrid:cycle:1` sets the order of one cycle of 168 days. Then a second seed, such as `daily:2026-09-23:1:0`, makes that day's board. Refer to [Sixfold](/docs/games/sixfold/).
...
- **Shared data lists are frozen.** The seed points into the Crosshatch shape list, the Serpentine poem list, and the Sixfold family schedule. You can add items to the end of a list. You cannot change the order of a list.
...
All six generators operate in the same pattern. The generator makes a random puzzle quickly. Then it examines the puzzle against quality limits. If the puzzle is not correct, the generator starts again. Each generator has a maximum number of tries.
