...
The number keys 3 to 10 are the Polygram level colors. The name keys are `crosshatch` (teal), `pierglass` (fuchsia), `doublet` (amber), `serpentine` (lime), `sixfold` (indigo), and `neutral` (black and white). The root accent is neutral black and white. The hub and the settings have no color accent. Each game surface sets its own `data-level`. The game surfaces are the game screen, the archive, the practice mode, and the stats.
